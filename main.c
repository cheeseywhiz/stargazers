...
uint8_t testData[100] = "Not Yet"; // DATA_SIZE
/* USER CODE END PV */
...
struct star_location get_star_location(const char *star, float lat, float lon) {
    ...
    HAL_UART_Receive(&hlpuart1, (void*)&location, sizeof(location), 0xFFFF);
    return location;
}
...
int main(void)
{
  ...
  MX_USART3_UART_Init();
  /* USER CODE BEGIN 2 */
  uint8_t testDataDisplay[100] = "Another"; // DATA_SIZE
  int success = 1;
  /* USER CODE END 2 */
  ...
  while (1)
  {
	...
	HAL_StatusTypeDef t = HAL_UART_Receive(&huart3, (uint8_t*) testData, 100, 5000); // DATA_SIZE
	if(t != HAL_OK){
		success = 0;
	}
	else{
